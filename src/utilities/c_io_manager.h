...
class c_io_manager : public QObject {
   ...
   public slots:
    ...
    QByteArray save(c_builder_view *builder,
                    const c_io_manager::jsonformat format = c_io_manager::jsonformat::database,
    ...
    bool load(c_builder_view *builder, const c_io_manager::jsonformat format,
              QString path = QString(), int id = 0);
    bool loadFrom(c_builder_view *builder, const c_io_manager::jsonformat format, QString path_json,
                  int id);
    void update(c_builder_view *builder,
                const c_io_manager::jsonformat format = c_io_manager::jsonformat::database,
                const QString path = QString());
