...
c_effect::c_effect(const c_dbmanager *dbmanager, int id, c_action action, int areaShape,
                   QVector<int> areaSize, QVector<float> params, QString description)
    : _id(id),
      _action(action),
...
int c_effect::getId() const { return _id; }

c_action c_effect::getAction() const { return _action; }

int c_effect::getAreaShape() const { return _areaShape; }

QVector<int> c_effect::getAreaSize() const { return _areaSize; }

QVector<float> c_effect::getParams() const { return _params; }
...
QString c_effect::getDescription() const { return _description; }

void c_effect::setId(const int id) { _id = id; }

void c_effect::setAction(const c_action Actionid) { _action = Actionid; }

void c_effect::setAreaShape(const int AreaShape) { _areaShape = AreaShape; }

void c_effect::setAreaSize(const QVector<int> AreaSize) { _areaSize = AreaSize; }

void c_effect::setParams(const QVector<float> Params) { _params = Params; }

QMap<QString, QString> c_effect::getEffectString(const int lvl) const {
    QMap<QString, QString> res;
    QString text = tokenizer.formatString(_action.getDescription(), _params.toList(), lvl,
                                          _action.getDescription().size());
    if (_description.isEmpty()) {
        if (text.toInt()) {
            c_state state = _database->get_state(text.toInt());
            res.insert("text",
                       QString("%1 (Niv. %2)").arg(state.get_name()).arg(double(_params.at(2))));
        ...
        QString descri = tokenizer.formatString(_description, _params.toList(), lvl,
                                                _action.getDescription().size());
...
void c_effect::setDescrition(const QString descri) { _description = descri; }
