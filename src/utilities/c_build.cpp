...
QMap<QString, int> c_build::getBonuses() const {
    ...
    bonus_global["Vie"] = int(qreal(bonuses["Vie"] + bonus_aptitudes["Point de Vie"] * 20) *
                              ((qreal(bonus_aptitudes["% Point de Vie"] * 4) / 100.0) + 1.0));
    ...
    bonus_global["Maîtrise Élémentaire Feu"] =
        bonuses["Maîtrise Élémentaire Feu"] + bonus_aptitudes["Maitrise Élémentaire"] * 5 +
        bonus_aptitudes["Point de Mouvement et dégâts"] * 20 +
        bonus_aptitudes["Portée et dégâts"] * 40 + bonus_aptitudes["Contrôle et dégâts"] * 40;
    bonus_global["Maîtrise Élémentaire Eau"] =
        bonuses["Maîtrise Élémentaire Eau"] + bonus_aptitudes["Maitrise Élémentaire"] * 5 +
        bonus_aptitudes["Point de Mouvement et dégâts"] * 20 +
        bonus_aptitudes["Portée et dégâts"] * 40 + bonus_aptitudes["Contrôle et dégâts"] * 40;
    bonus_global["Maîtrise Élémentaire Terre"] =
        bonuses["Maîtrise Élémentaire Terre"] + bonus_aptitudes["Maitrise Élémentaire"] * 5 +
        bonus_aptitudes["Point de Mouvement et dégâts"] * 20 +
        bonus_aptitudes["Portée et dégâts"] * 40 + bonus_aptitudes["Contrôle et dégâts"] * 40;
    bonus_global["Maîtrise Élémentaire Air"] =
        bonuses["Maîtrise Élémentaire Air"] + bonus_aptitudes["Maitrise Élémentaire"] * 5 +
        bonus_aptitudes["Point de Mouvement et dégâts"] * 20 +
        bonus_aptitudes["Portée et dégâts"] * 40 + bonus_aptitudes["Contrôle et dégâts"] * 40;
    bonus_global["Résistance Feu"] = bonuses["Résistance Feu"] +
                                     bonus_aptitudes["Résistance Élémentaire"] * 10 +
                                     bonus_aptitudes["Résistance Élémentaire majeur"] * 50;
    bonus_global["Résistance Eau"] = bonuses["Résistance Eau"] +
                                     bonus_aptitudes["Résistance Élémentaire"] * 10 +
                                     bonus_aptitudes["Résistance Élémentaire majeur"] * 50;
    bonus_global["Résistance Terre"] = bonuses["Résistance Terre"] +
                                       bonus_aptitudes["Résistance Élémentaire"] * 10 +
                                       bonus_aptitudes["Résistance Élémentaire majeur"] * 50;
    bonus_global["Résistance Air"] = bonuses["Résistance Air"] +
                                     bonus_aptitudes["Résistance Élémentaire"] * 10 +
                                     bonus_aptitudes["Résistance Élémentaire majeur"] * 50;
    bonus_global["Dommages infligés"] =
        bonuses["Dommages infligés"] + bonus_aptitudes["% Dommages infligés"] * 10;
    ...
    bonus_global["Esquive"] = bonuses["Esquive"] + bonus_aptitudes["Esquive"] * 6 +
                              bonus_aptitudes["Tacle et Esquive"] * 4;
    bonus_global["Tacle"] =
        bonuses["Tacle"] + bonus_aptitudes["Tacle"] * 6 + bonus_aptitudes["Tacle et Esquive"] * 4;
    ...
    bonus_global["Maîtrise Critique"] =
        bonuses["Maîtrise Critique"] + bonus_aptitudes["Maîtrise Critique"] * 4;
    bonus_global["Résistance Critique"] =
        bonuses["Résistance Critique"] + bonus_aptitudes["Résistance Critique"] * 4;
    bonus_global["Maîtrise Dos"] = bonuses["Maîtrise Dos"] + bonus_aptitudes["Maîtrise Dos"] * 6;
    bonus_global["Résistance Dos"] =
        bonuses["Résistance Dos"] + bonus_aptitudes["Résistance Dos"] * 4;
    bonus_global["Maîtrise Mêlée"] =
        bonuses["Maîtrise Mêlée"] + bonus_aptitudes["Maitrise Mêlée"] * 8;
    bonus_global["Maîtrise Distance"] =
        bonuses["Maîtrise Distance"] + bonus_aptitudes["Maitrise Distance"] * 8;
    bonus_global["Maîtrise Monocible"] =
        bonuses["Maîtrise Monocible"] + bonus_aptitudes["Maitrise Monocible"] * 8;
    ...
    bonus_global["Maîtrise Berserk"] =
        bonuses["Maîtrise Berserk"] + bonus_aptitudes["Maîtrise Berserk"] * 8;
    ...
    bonus_global["Armure"] =
        int(qreal(bonuses["Vie"]) * qreal(bonus_aptitudes["% Point de Vie en Armure"] * 4) / 100.0);
...
void c_build::setBonuses(const QMap<QString, int> &value) { bonuses = value; }

QList<QString> c_build::getElements() const { return elements; }

void c_build::setElements(const QList<QString> &value) { elements = value; }
...
QMap<QString, c_item> c_build::getEquipment() const { return equipment; }

void c_build::setEquipment(const QMap<QString, c_item> &value) { equipment = value; }
...
QMap<QString, QString> c_build::equip(const c_item &item) {
    ...
    if (!type.compare("SECOND_HAND")) {
        if (!equipment["FIRST_WEAPON"].isEmpty()) {
            if (!mapItemToType[equipment["FIRST_WEAPON"].getType().getTitle()].compare(
                    "TWO_HAND_WEAPON")) {
...
QMap<QString, QString> c_build::check_constraints(const c_item &item) {
    ...
    if (lvl + bonuses["Art du barda"] < item.getLvl()) {
        ...
        res["message"] =
            "Vous ne pouvez pas équiper cet objet : son niveau est trop élevé par rapport à votre "
            "build";
        return res;
    }
...
bool c_build::getEpique_free() const { return epique_free; }

void c_build::setEpique_free(bool value) { epique_free = value; }

bool c_build::getRelique_free() const { return relique_free; }

void c_build::setRelique_free(bool value) { relique_free = value; }
...
QMap<QString, int> c_build::resetMap() {
    bonuses.clear();
    bonuses["Vie"] =
        c_calcul::compute_life(lvl) + (guilde_bonus ? 55 : 0) + (nation_bonus ? 20 : 0);
    bonuses["PA"] = 6;
    bonuses["PM"] = 3;
    bonuses["PW"] = 6;
...
int c_build::getLvl() const { return lvl; }

QMap<QString, int> c_build::getBonus_aptitudes() const { return bonus_aptitudes; }
