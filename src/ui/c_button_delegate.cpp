...
c_button_delegate::c_button_delegate(QObject *parent) : QStyledItemDelegate(parent) {
    if (QTableView *tableView = qobject_cast<QTableView *>(parent)) {
        ...
        btn->setStyleSheet(
            "border-top:1px solid #BBBBBB; border-bottom:1px solid #BBBBBB; background-color: "
            "rgba(255, 255, 255, 0);");
...
QWidget *c_button_delegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option,
...
// setEditorData
void c_button_delegate::setEditorData(QWidget *editor, const QModelIndex &index) const {
    if (index.column() == column_delete) {
...
// paint
void c_button_delegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
                              const QModelIndex &index) const {
    if (index.column() == column_delete) {
...
// updateGeometry
void c_button_delegate::updateEditorGeometry(QWidget *editor, const QStyleOptionViewItem &option,
                                             const QModelIndex &index) const {
    QRect rect = option.rect;
...
void c_button_delegate::setModel(c_save_builder_model *value) { model = value; }
