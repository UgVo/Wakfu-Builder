...
c_enchantement_item::c_enchantement_item(QWidget *parent, QString emplacement)
    ...
    ui->widget_4->setStyleSheet(
        QString("#widget_4 {background-color : %1; border : 1px solid %2; border-radius : 3px;}")
            .arg(app_color::dark_blue)
            .arg(app_color::grey_blue_3));
    QObject::connect(ui->toolButton, &QToolButton::clicked, this,
                     &c_enchantement_item::slot_positionClicked);
    ...
    foreach (c_tool_button *button, _tool_buttons) {
        QObject::connect(button, &QToolButton::clicked, this,
                         &c_enchantement_item::slot_slotClicked);
        QObject::connect(button, &c_tool_button::rightClicked, this,
                         &c_enchantement_item::slot_slotRightClicked);
...
c_enchantement_item::~c_enchantement_item() { delete ui; }
...
void c_enchantement_item::updateView() {
    if (_emplacement.contains("HAND")) {
        ...
        ui->toolButton->setIcon(
            QIcon(QString(":/images/divers/small%1.png").arg(_emplacement.toLower())));
    }
    ...
    for (int i = 0; i < _chasse_list.size(); ++i) {
        if (_chasse_list[i].color()) {
            ...
            if (_chasse_list[i].white()) {
                ...
                _tool_buttons[i]->setIcon(
                    QIcon(QString(":/images/shard/shard%1.png").arg(_chasse_list[i].color())));
            }
...
void c_enchantement_item::slot_highlightPosition(bool flag) {
    if (flag) {
        ui->widget_4->setStyleSheet(
            QString(
                "#widget_4 {background-color : %1; border : 2px solid white; border-radius : 3px;}")
                .arg(app_color::dark_blue));
    } else {
        ui->widget_4->setStyleSheet(
            QString(
                "#widget_4 {background-color : %1; border : 1px solid %2; border-radius : 3px;}")
                .arg(app_color::dark_blue)
                .arg(app_color::grey_blue_3));
    }
}

void c_enchantement_item::slot_positionClicked() { emit positionSelected(_emplacement); }
...
void c_enchantement_item::slot_slotClicked() {
    ...
    c_enchantement_effect effect =
        static_cast<c_enchantement_display *>(_parent)->selected_effect();
...
int c_enchantement_item::level() const { return _level; }

QList<c_enchantement_chasse> c_enchantement_item::chasse_list() const { return _chasse_list; }

QString c_enchantement_item::emplacement() const { return _emplacement; }
