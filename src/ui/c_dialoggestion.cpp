...
DialogGestion::DialogGestion(c_datamanager *data, QString ver, QWidget *parent)
    ...
    connect(ui->pb_update_data_base, &QPushButton::clicked, this,
            &DialogGestion::slot_update_database);
    ...
    if (version_file == version) {
        ui->textBrowser->append(
            QString("Actual version of files up to date %1 \nUpdate files button disabled")
                .arg(version));
    }
    ui->progressBar->setValue(0);
}

DialogGestion::~DialogGestion() { delete ui; }

void DialogGestion::setVersionText(QString textVer) { ui->label_version->setText(textVer); }
...
void DialogGestion::slot_update_file() {
    ...
    connect(data_manager, &c_datamanager::downloadFileFinished, this,
            &DialogGestion::slot_endUpdateFiles);
    ui->progressBar->setValue(0);
...
void DialogGestion::setText(QString txt) { ui->label->setText(txt); }
...
void DialogGestion::slot_update_database() {
    ...
    connect(data_manager, &c_datamanager::updateItemFinished, this,
            &DialogGestion::slot_endUpdateItems);
    parser->start();
}
...
void DialogGestion::slot_endUpdateItems() {
    ...
    disconnect(data_manager, &c_datamanager::updateItemFinished, this,
               &DialogGestion::slot_endUpdateItems);
    ui->progressBar->setValue(ui->progressBar->maximum());
    ui->pb_update_files->setEnabled(true);
    ui->pb_update_data_base->setEnabled(true);
...
void DialogGestion::slot_update_images() {
    ...
    connect(data_manager, &c_datamanager::downloadImageFinished, this,
            &DialogGestion::slot_endUpdateImages);
    ui->progressBar->setValue(0);
    ui->progressBar->setMinimum(0);
    ui->progressBar->setMaximum(0);
    ui->pb_update_files->setEnabled(false);
...
void DialogGestion::slot_endUpdateImages() {
    ...
    disconnect(data_manager, &c_datamanager::downloadImageFinished, this,
               &DialogGestion::slot_endUpdateImages);
    ui->progressBar->setValue(ui->progressBar->maximum());
    ui->pb_update_files->setEnabled(true);
    ui->pb_update_data_base->setEnabled(true);
...
void DialogGestion::set_version(QString _version) { version = _version; }
