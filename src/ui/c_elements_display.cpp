...
c_elements_display::c_elements_display(QWidget *parent)
    : QWidget(parent), ui(new Ui::c_elements_display) {
    ui->setupUi(this);
...
c_elements_display::~c_elements_display() { delete ui; }
...
void c_elements_display::update_view() {
    for (int i = 0; i < 3; ++i) {
        label_list[i]->setPixmap(
            QPixmap(QString(":/images/elements/%1_big.png").arg(frToEn_elem[chosen_elements[i]])));
        label_list[i]->setBackgroundRole(QPalette::Base);
        label_list[i]->setScaledContents(true);
    }
    label_list[3]->setPixmap(
        QPixmap(QString(":/images/elements/%1_inked.png").arg(frToEn_elem[chosen_elements[3]])));
...
void c_elements_display::mouseDoubleClickEvent(QMouseEvent * /*event*/) { emit doubleCliked(); }
