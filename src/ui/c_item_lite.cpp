...
c_item_lite::c_item_lite(c_dbmanager *database_manager, c_item item, QWidget *parent)
    : QWidget(parent), ui(new Ui::c_item_lite) {
    ui->setupUi(this);
    this->setAttribute(Qt::WA_Hover);
    _parent = parent;
    _item = item;
    setStyleSheet(
        QString(".c_item_lite {background-color: %1; border: 1px solid %2; border-radius: 4px;}")
            .arg(app_color::dark_blue)
            .arg(app_color::color_rarity.at(_item.getRarity())));
    ui->widget_background->setStyleSheet(QString(".QWidget#widget_background{background-color: %1; "
                                                 "border : 1px solid %1; border-radius: 4px;}")
                                             .arg(app_color::dark_blue));
    ui->lvl->setStyleSheet(QString("background-color: %1; border: 1px solid white; border-radius: "
                                   "5px; color:white; padding: 2px ")
                               .arg(app_color::dark));
    ui->image_widget->setStyleSheet(
        QString(".QWidget#image_widget{background-color:%1; border-top-left-radius: 3px; "
...
void c_item_lite::setDatabase_manager(c_dbmanager *manager) { _database_manager = manager; }

c_item c_item_lite::item() const { return _item; }
...
void c_item_lite::mouseReleaseEvent(QMouseEvent *event) { mouseDoubleClickEvent(event); }
...
void c_item_lite::updateView() {
    ...
    QString imagePath = QString(c_datamanager::imageDirectory() + "%1.png").arg(_item.getGfxId());
