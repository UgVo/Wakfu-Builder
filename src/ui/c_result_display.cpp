...
c_result_display::c_result_display(c_dbmanager *_data_base, QWidget *parent)
    : QWidget(parent), ui(new Ui::c_result_display) {
    ui->setupUi(this);
    this->setStyleSheet(QString("c_result_display{background-color : %1;} "
                                "c_item_lite{background-color : %2}")
...
c_result_display::~c_result_display() { delete ui; }
...
void c_result_display::slot_new_search_result(QList<int> item_id_list) {
    ...
    used_page_number =
        qCeil(qreal(item_id_list.size()) / qreal((ROW_PER_PAGE * COLUMN_PER_PAGE))) - 1;
...
void c_result_display::fill_page(int page) {
    ...
    sub_item_id_list =
        id_list.mid(ROW_PER_PAGE * COLUMN_PER_PAGE * page, ROW_PER_PAGE * COLUMN_PER_PAGE);
    ...
    for (int i = 0; i < size / n; ++i) {
        ...
        QObject::connect(item_lite_list.last(), &c_item_lite::item_doubleCliked, this,
                         &c_result_display::slot_item_doubleCliked);
        QObject::connect(item_lite_list.last(), &c_item_lite::item_hovered, this,
                         &c_result_display::slot_item_hovered);
        QObject::connect(item_lite_list.last(), &c_item_lite::item_hide, this,
                         &c_result_display::slot_item_hide);
    ...
    for (int j = 0; j < item_list.size(); ++j) {
        ...
        static_cast<QGridLayout *>(stackedWidget->widget(page)->layout())
            ->addWidget(item_lite_list.at(j), i / COLUMN_PER_PAGE, i % COLUMN_PER_PAGE);
        ++i;
    }
    qDebug() << "Creation of shadows and added to the widget :" << timer.elapsed()
             << "ms : " << stackedWidget->widget(page)->layout()->count() << " elements";
    timer.restart();
    QSpacerItem *spacer = new QSpacerItem(20, 40, QSizePolicy::Expanding, QSizePolicy::Expanding);
    static_cast<QGridLayout *>(stackedWidget->widget(page)->layout())
        ->addItem(spacer, ROW_PER_PAGE, COLUMN_PER_PAGE);
    stackedWidget->widget(page)->show();
    ui->page_indicator->setText(
        QString("%1 - %2 de %3")
            .arg(current_page * COLUMN_PER_PAGE * ROW_PER_PAGE + 1)
            .arg(sub_item_id_list.size() + page * COLUMN_PER_PAGE * ROW_PER_PAGE)
            .arg(number_items));
    loading->hide();
}
...
void c_result_display::slot_next() {
    ...
    int upper_bound =
        (current_page * COLUMN_PER_PAGE * ROW_PER_PAGE + COLUMN_PER_PAGE * ROW_PER_PAGE >
                 number_items
             ? number_items
             : current_page * COLUMN_PER_PAGE * ROW_PER_PAGE + COLUMN_PER_PAGE * ROW_PER_PAGE);
    ui->page_indicator->setText(
        QString("%1 - %2 de %3")
            .arg(number_items ? current_page * COLUMN_PER_PAGE * ROW_PER_PAGE + 1 : 0)
            .arg(upper_bound)
            .arg(number_items));
}
...
void c_result_display::slot_item_doubleCliked(c_item item) { emit item_doubleCliked(item); }

void c_result_display::slot_item_hovered(QString type, QPoint pos) { emit item_hovered(type, pos); }

void c_result_display::slot_item_hide(QString type) { emit item_hide(type); }
