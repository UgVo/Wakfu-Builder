...
c_builder_view::c_builder_view(c_dbmanager *_manager, QCompleter *search_completer,
                               QWidget *_parent)
    : QWidget(_parent),
    ...
    QObject::connect(search_widget, &c_search_widget::new_search_result, result_display,
                     &c_result_display::slot_new_search_result);
    QObject::connect(search_widget, &c_search_widget::new_search_result_sorted, result_display,
                     &c_result_display::slot_new_search_result_sorted);
    QObject::connect(result_display, &c_result_display::item_doubleCliked, build_display,
                     &c_build_display::equip_new_item);
    QObject::connect(status_build, &c_status_build::lvl_changed, search_widget,
                     &c_search_widget::setLvl);
    QObject::connect(status_build, &c_status_build::lvl_changed, build, &c_build::setLvl);
    QObject::connect(status_build, &c_status_build::lvl_changed, aptitude_display,
                     &c_aptitudes_display::setLvl);
    QObject::connect(status_build, &c_status_build::lvl_changed, enchantement_display,
                     &c_enchantement_display::slot_setLevel);
    QObject::connect(status_build, &c_status_build::bonus_changed, build,
                     &c_build::slot_bonus_changed);
    QObject::connect(element_display, &c_elements_display::newElements, build,
                     &c_build::setElements);
    QObject::connect(build_display, &c_build_display::load_search_position, search_widget,
                     &c_search_widget::slot_load_search_position);
    QObject::connect(aptitude_display, &c_aptitudes_display::value_changed, build,
                     &c_build::slot_aptitude_value_changed);
    // QObject::connect(result_display, &c_result_display::item_hovered, build_display,
    //                  &c_build_display::slot_item_hovered);
    // QObject::connect(result_display, &c_result_display::item_hide, build_display,
    //                  &c_build_display::slot_item_hide);
    QObject::connect(status_build, &c_status_build::show_class_popup, this,
                     &c_builder_view::slot_show_class_popup);
    QObject::connect(build, &c_build::updated, calcul, &c_calcul::computeMainMastery);
    QObject::connect(element_display, &c_elements_display::doubleCliked, this,
                     &c_builder_view::slot_show_element_popup);
    ...
    ui->widget->setStyleSheet(
        QString("QWidget#widget{background-color:%1}").arg(app_color::grey_blue));
...
c_builder_view::~c_builder_view() { delete ui; }

c_status_build *c_builder_view::getStatus_build() const { return status_build; }

c_build_display *c_builder_view::getBuild_display() const { return build_display; }

c_elements_display *c_builder_view::getElement_display() const { return element_display; }

int c_builder_view::getId() const { return id; }

void c_builder_view::setId(int value) { id = value; }

QString c_builder_view::getPath() const { return path; }

void c_builder_view::setPath(const QString &value) { path = value; }

MainWindow *c_builder_view::getParent() const { return parent; }

void c_builder_view::resizeEvent(QResizeEvent * /*event*/) {
    // Screen size handling
    if ((this->rect().width() < 1780 && state_column_number == -1) ||
        (this->rect().width() < 1780 && state_column_number == 3)) {
    ...
    } else if ((this->rect().width() >= 1780 && state_column_number == -1) ||
               (this->rect().width() >= 1780 && state_column_number == 2)) {
    ...
    switch (state_element) {
        case 0:
            if (element_popup != nullptr) {
                QPoint elem_popup_pos =
                    QPoint((rect().width() - element_popup->width()) / 2, -element_popup->height());
        ...
        case 1:
            if (element_popup != nullptr) {
                QPoint elem_popup_pos = QPoint((rect().width() - element_popup->width()) / 2,
                                               (rect().height() - element_popup->height()) / 3);
                element_popup->move(elem_popup_pos);
            }
            break;
        default:
    ...
    QPoint tc_pos = QPoint(rect().width() - tc_resume->width() - 20,
                           rect().height() - tc_resume->height() - 20);
...
c_aptitudes_display *c_builder_view::getAptitude_display() const { return aptitude_display; }
...
void c_builder_view::slot_show_element_popup() {
    if (element_popup != nullptr) {
        QObject::disconnect(element_popup, &c_element_popup_widget::accepted, this,
                            &c_builder_view::slot_hide_element_popup);
        element_popup->deleteLater();
        element_popup = nullptr;
    }
    element_popup = new c_element_popup_widget(this);
    element_popup->show();
    QObject::connect(element_popup, &c_element_popup_widget::accepted, this,
                     &c_builder_view::slot_hide_element_popup);
    status_build->setDisabled(true);
    build_display->setDisabled(true);
    element_display->setDisabled(true);
    ...
    QPoint elem_popup_pos =
        QPoint((rect().width() - element_popup->width()) / 2, -element_popup->height());
    QPoint elem_popup_pos_final = QPoint((rect().width() - element_popup->width()) / 2,
                                         (rect().height() - element_popup->height()) / 3);
...
void c_builder_view::slot_show_class_popup() {
    if (class_selection_popup != nullptr) {
        QObject::disconnect(class_selection_popup, &c_class_selection::class_chosen, this,
                            &c_builder_view::slot_hide_class_popup);
        class_selection_popup->deleteLater();
        class_selection_popup = nullptr;
    }
    class_selection_popup = new c_class_selection(this);
    class_selection_popup->show();
    QObject::connect(class_selection_popup, &c_class_selection::class_chosen, status_build,
                     &c_status_build::slot_class_changed);
    QObject::connect(class_selection_popup, &c_class_selection::class_chosen, this,
                     &c_builder_view::slot_hide_class_popup);
    QObject::connect(class_selection_popup, &c_class_selection::rejected, this,
                     &c_builder_view::slot_hide_class_popup);
    status_build->setDisabled(true);
    build_display->setDisabled(true);
    element_display->setDisabled(true);
    ...
    search_widget->setDisabled(true);
    ui->tabWidget->setDisabled(true);
    QPoint class_popup_pos = QPoint((rect().width() - class_selection_popup->width()) / 2,
                                    -class_selection_popup->height());
    QPoint class_popup_pos_final = QPoint((rect().width() - class_selection_popup->width()) / 2,
                                          (rect().height() - class_selection_popup->height()) / 3);
