...
c_status_build::c_status_build(QWidget *parent) : QWidget(parent), ui(new Ui::c_status_build) {
    ui->setupUi(this);

    setStyleSheet(QString("c_status_build{background-color: %1; color:white} QLabel{color:white;} "
                          "QLineEdit{color : %2;} QSpinBox{color : %2;}")
    ...
    ui->guild_cb->setStyleSheet(
        "QCheckBox::indicator:unchecked {image: url(:/images/divers/65_inked.png);}"
        "QCheckBox::indicator:checked {image: url(:/images/divers/65.png);} ");
    ui->guild_cb->setToolTip("Bonus de guilde");
    ui->nation_cb->setStyleSheet(
        "QCheckBox::indicator:unchecked {image: url(:/images/divers/6_inked.png);}"
        "QCheckBox::indicator:checked {image: url(:/images/divers/6.png);} ");
    ui->nation_cb->setToolTip("Bonus de nation");
    ui->hm_cb->setStyleSheet(
        "QCheckBox::indicator:unchecked {image: url(:/images/divers/9_inked.png);}"
        "QCheckBox::indicator:checked {image: url(:/images/divers/9.png);} ");
    ui->hm_cb->setToolTip("Bonus de Havre-monde");

    ui->bonuses->setStyleSheet(QString("background-color : %1; border-radius: 3px; color:white")
    ...
    ui->widget_name_lvl->setStyleSheet(
    ...
    QObject::connect(ui->build_lvl, &QSpinBox::editingFinished, this,
                     &c_status_build::slot_enter_pressed);
    ...
    QObject::connect(ui->button_class, &QToolButton::clicked, this,
                     &c_status_build::slot_class_clicked);
}

c_status_build::~c_status_build() { delete ui; }

int c_status_build::getClasse() const { return classe; }
...
QString c_status_build::getName() { return ui->build_name->text(); }

int c_status_build::getLvl() { return ui->build_lvl->value(); }

void c_status_build::setName(QString name) { ui->build_name->setText(name); }

bool c_status_build::isActivated_HM() { return ui->hm_cb->isChecked(); }

bool c_status_build::isActivated_Nation() { return ui->nation_cb->isChecked(); }

bool c_status_build::isActivated_Guilde() { return ui->guild_cb->isChecked(); }
...
void c_status_build::slot_class_clicked() { emit show_class_popup(); }
