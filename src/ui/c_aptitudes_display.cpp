...
c_aptitudes_display::c_aptitudes_display(QWidget *_parent)
    : QWidget(_parent), ui(new Ui::c_aptitudes_display) {
    ...
    ui->intell_count->setStyleSheet(QString("background-color: %1; border: 1px solid white; "
                                            "border-radius: 8px; color:white; padding: 2px ")
                                        .arg(app_color::dark));
    ui->chance_count->setStyleSheet(QString("background-color: %1; border: 1px solid white; "
                                            "border-radius: 8px; color:white; padding: 2px ")
                                        .arg(app_color::dark));
    ui->agil_count->setStyleSheet(QString("background-color: %1; border: 1px solid white; "
                                          "border-radius: 8px; color:white; padding: 2px ")
                                      .arg(app_color::dark));
    ui->force_count->setStyleSheet(QString("background-color: %1; border: 1px solid white; "
                                           "border-radius: 8px; color:white; padding: 2px ")
                                       .arg(app_color::dark));
    ui->majeur_count->setStyleSheet(QString("background-color: %1; border: 1px solid white; "
                                            "border-radius: 8px; color:white; padding: 2px ")
                                        .arg(app_color::dark));

    ui->widget_agil->setStyleSheet(
        QString("background-color: %1; border-radius: 3px; ").arg(app_color::dark_blue));
    ui->widget_force->setStyleSheet(
        QString("background-color: %1; border-radius: 3px; ").arg(app_color::dark_blue));
    ui->widget_chance->setStyleSheet(
        QString("background-color: %1; border-radius: 3px; ").arg(app_color::dark_blue));
    ui->widget_intell->setStyleSheet(
        QString("background-color: %1; border-radius: 3px; ").arg(app_color::dark_blue));
    ui->widget_majeur->setStyleSheet(
        QString("background-color: %1; border-radius: 3px; ").arg(app_color::dark_blue));

    map_unites["intelligence"] = new QList<c_aptitude_unite *>;
    map_unites["intelligence"]->push_back(
        new c_aptitude_unite("% Point de Vie", -1, "big_vie_100", "intelligence", this));
    ui->layout_intell->insertWidget(1, map_unites["intelligence"]->last());
    map_unites["intelligence"]->push_back(
        new c_aptitude_unite("Résistance Élémentaire", 10, "big_resistance", "intelligence", this));
    ui->layout_intell->insertWidget(2, map_unites["intelligence"]->last());
    map_unites["intelligence"]->push_back(
        new c_aptitude_unite("Barrière", 10, "big_barriere", "intelligence", this));
    ui->layout_intell->insertWidget(3, map_unites["intelligence"]->last());
    map_unites["intelligence"]->push_back(
        new c_aptitude_unite("% Soin Reçu", 5, "big_soin", "intelligence", this));
    ui->layout_intell->insertWidget(4, map_unites["intelligence"]->last());
    map_unites["intelligence"]->push_back(
        new c_aptitude_unite("% Point de Vie en Armure", 10, "big_armure", "intelligence", this));
    ui->layout_intell->insertWidget(5, map_unites["intelligence"]->last());

    map_unites["force"] = new QList<c_aptitude_unite *>;
    map_unites["force"]->push_back(
        new c_aptitude_unite("Maitrise Élémentaire", -1, "big_elementaire", "force", this));
    ui->layout_force->insertWidget(1, map_unites["force"]->last());
    map_unites["force"]->push_back(
        new c_aptitude_unite("Maitrise Monocible", 20, "big_monocible", "force", this));
    ui->layout_force->insertWidget(2, map_unites["force"]->last());
    map_unites["force"]->push_back(
        new c_aptitude_unite("Maitrise Zone", 20, "big_zone", "force", this));
    ui->layout_force->insertWidget(3, map_unites["force"]->last());
    map_unites["force"]->push_back(
        new c_aptitude_unite("Maitrise Mêlée", 20, "big_melee", "force", this));
    ui->layout_force->insertWidget(4, map_unites["force"]->last());
    map_unites["force"]->push_back(
        new c_aptitude_unite("Maitrise Distance", 20, "big_distance", "force", this));
    ui->layout_force->insertWidget(5, map_unites["force"]->last());
    map_unites["force"]->push_back(
        new c_aptitude_unite("Point de Vie", -1, "big_point_vie", "force", this));
    ui->layout_force->insertWidget(6, map_unites["force"]->last());

    map_unites["agilite"] = new QList<c_aptitude_unite *>;
    map_unites["agilite"]->push_back(
        new c_aptitude_unite("Tacle", -1, "big_tacle", "agilite", this));
    ui->layout_agil->insertWidget(1, map_unites["agilite"]->last());
    map_unites["agilite"]->push_back(
        new c_aptitude_unite("Esquive", -1, "big_esquive", "agilite", this));
    ui->layout_agil->insertWidget(2, map_unites["agilite"]->last());
    map_unites["agilite"]->push_back(
        new c_aptitude_unite("Initiative", 20, "big_initiative", "agilite", this));
    ui->layout_agil->insertWidget(3, map_unites["agilite"]->last());
    map_unites["agilite"]->push_back(
        new c_aptitude_unite("Tacle et Esquive", -1, "big_tacle_esquive", "agilite", this));
    ui->layout_agil->insertWidget(4, map_unites["agilite"]->last());
    map_unites["agilite"]->push_back(
        new c_aptitude_unite("Volonté", 20, "big_volonte", "agilite", this));
    ui->layout_agil->insertWidget(5, map_unites["agilite"]->last());

    map_unites["chance"] = new QList<c_aptitude_unite *>;
    map_unites["chance"]->push_back(
        new c_aptitude_unite("% Coup Critique", 20, "big_critique", "chance", this));
    ui->layout_chance->insertWidget(1, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("% Parade", 20, "big_parade", "chance", this));
    ui->layout_chance->insertWidget(2, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("Maîtrise Critique", -1, "big_critique_m", "chance", this));
    ui->layout_chance->insertWidget(3, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("Maîtrise Dos", -1, "big_dos_m", "chance", this));
    ui->layout_chance->insertWidget(4, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("Maîtrise Berserk", -1, "big_berserk", "chance", this));
    ui->layout_chance->insertWidget(5, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("Maîtrise Soin", -1, "big_soin_m", "chance", this));
    ui->layout_chance->insertWidget(6, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("Résistance Dos", 20, "big_dos_m", "chance", this));
    ui->layout_chance->insertWidget(7, map_unites["chance"]->last());
    map_unites["chance"]->push_back(
        new c_aptitude_unite("Résistance Critique", 20, "big_critique_r", "chance", this));
    ui->layout_chance->insertWidget(8, map_unites["chance"]->last());

    map_unites["majeur"] = new QList<c_aptitude_unite *>;
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("Point d'action", 1, "big_pa", "majeur", this));
    ui->layout_majeur->insertWidget(1, map_unites["majeur"]->last());
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("Point de Mouvement et dégâts", 1, "big_pm", "majeur", this));
    ui->layout_majeur->insertWidget(2, map_unites["majeur"]->last());
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("Portée et dégâts", 1, "big_porte", "majeur", this));
    ui->layout_majeur->insertWidget(3, map_unites["majeur"]->last());
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("Point de Wakfu", 1, "big_wakfu", "majeur", this));
    ui->layout_majeur->insertWidget(4, map_unites["majeur"]->last());
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("Contrôle et dégâts", 1, "big_controle", "majeur", this));
    ui->layout_majeur->insertWidget(5, map_unites["majeur"]->last());
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("% Dommages infligés", 1, "big_dommage", "majeur", this));
    ui->layout_majeur->insertWidget(6, map_unites["majeur"]->last());
    map_unites["majeur"]->push_back(
        new c_aptitude_unite("Résistance Élémentaire majeur", 1, "big_resistance", "majeur", this));
    ...
    foreach (QString key, map_unites.keys()) {
        ...
        foreach (c_aptitude_unite *unite, list) {
            QObject::connect(parent->getParent(), &MainWindow::shift_pressed, unite,
                             &c_aptitude_unite::shift_pressed);
            QObject::connect(unite, &c_aptitude_unite::value_changed, this,
                             &c_aptitudes_display::slot_value_changed);
...
c_aptitudes_display::~c_aptitudes_display() { delete ui; }
