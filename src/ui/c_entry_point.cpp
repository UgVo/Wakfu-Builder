...
c_entry_point::c_entry_point(c_dbmanager *_manager, QWidget *_parent)
    ...
    QObject::connect(ui->pushButton_new_build, &QPushButton::clicked, this,
                     &c_entry_point::slot_new_button);
    QObject::connect(ui->pushButton_open_build, &QPushButton::clicked, this,
                     &c_entry_point::slot_open_button);
    ...
    movie->start();
    static_cast<QStackedLayout *>(ui->stackedWidget_entry_point->layout())
...
c_entry_point::~c_entry_point() { delete ui; }
...
void c_entry_point::resizeEvent(QResizeEvent * /*event*/) {
    ...
    ui->background_eliacube->move((rect.width() - movie_size.width()) / 2,
                                  (rect.height() - movie_size.height()) / 2);
    ...
    if (state == 0) {
        ...
        p_label_creation_builder =
            QPoint((rect.width() - 106) / 2, (rect.height() - 172) / 2 + 156);
        ...
        p_w_bdd =
            QPoint((rect.width() - 1210) / 2 + 610, (rect.height() - 150 - 400 + 20) / 2 + 170);
        p_pb_new_build = QPoint(20, 20);
        p_pb_open_build = QPoint((rect.width() - 250) / 2, (rect.height() - 150 - 400 + 20) / 2);
        p_label_creation_builder =
            QPoint((rect.width() - 120) / 2, (rect.height() - 172) / 2 + 156);
    }
...
void c_entry_point::slot_open_button() {
    if (state == 0) {
        ...
        if (load_builder == nullptr) {
            ...
            load_builder->setStyleSheet(
                QString("c_load_builder_dialog {background-color:%1; border-radius:3px;}")
            ...
            QObject::connect(load_builder, &c_load_builder_dialog::accepted, this,
                             &c_entry_point::slot_open_from_source);
        }
        if (file_dial == nullptr) {
            file_dial = new QFileDialog(this, tr("Save Build"), parent->getApp_path() + "/save",
                                        tr("Json files (*.json)"));
            file_dial->setStyleSheet(
                QString("QFileDialog{ background-color:%1; border-radius:3px;}")
                    .arg(app_color::grey_blue));
            ...
            QObject::connect(file_dial, &QFileDialog::accepted, this,
                             &c_entry_point::slot_open_from_source);
        }

        QRect rect = this->rect();
        QPoint p_w_file((rect.width() - 1210) / 2, rect.height());
        QPoint p_w_file_final((rect.width() - 1210) / 2,
                              (rect.height() - 150 - 400 + 20) / 2 + 170);
        QPoint p_w_bdd((rect.width() - 1210) / 2 + 610, rect.height());
        QPoint p_w_bdd_final((rect.width() - 1210) / 2 + 610,
                             (rect.height() - 150 - 400 + 20) / 2 + 170);
        ...
        QPoint p_pb_open_build_final((rect.width() - 250) / 2,
                                     (rect.height() - 150 - 400 + 20) / 2);
        ...
        QObject::connect(load_builder, &c_load_builder_dialog::rejected, this,
                         &c_entry_point::slot_cancel_open);
    }
}
...
void c_entry_point::slot_new_button() {
    if (state == 0) {
        ...
        QObject::connect(parralle_anim, &QParallelAnimationGroup::finished, this,
                         &c_entry_point::slot_first_anim_finished);
...
void c_entry_point::slot_creation_builder_anim() {
    ...
    QObject::connect(parralle_anim, &QParallelAnimationGroup::finished, this,
                     &c_entry_point::slot_second_anim_finished);
}

void c_entry_point::slot_second_anim_finished() { emit second_animation_finished(); }
...
void c_entry_point::slot_open_from_source() {
    ...
    ui->label_creation_builder->hide();
    QObject::connect(parralle_anim, &QParallelAnimationGroup::finished, ui->pushButton_new_build,
                     &QPushButton::hide);
    QObject::connect(parralle_anim, &QParallelAnimationGroup::finished, ui->label_creation_builder,
                     &QLabel::show);
    QObject::connect(parralle_anim, &QParallelAnimationGroup::finished, this,
                     &c_entry_point::slot_open_animation_finished);
}
